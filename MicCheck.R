...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      checkboxGroupInput(
        "Q1c",
        "Did you use LEDs or lasers to illuminate your sample?",
        choiceNames = list("LEDs", "Lasers"),
    ...
    mainPanel(
      ...
      tags$head(
        tags$style(
          "#c1,#c2,#c3,#c4,#c5,#c5,#c6,#c7,#c8{color: white;
                           font-style: bold;
...
### Title generation  
  ...
  output$c1 <- renderText({
    scope <- getModalityName(input$Q1)
    paste0("Microscope Stand and Motorized Components")
  })
...
#### Conditional checklist categories
  ...
  output$Stand_motorized <- renderUI({
    ...
    HTML(paste(sort(all,decreasing=TRUE)))
  })
